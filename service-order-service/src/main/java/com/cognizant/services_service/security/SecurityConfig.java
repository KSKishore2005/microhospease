 ...
 * {@code X-Auth-Role} headers. This service:
 *
 * <ol>
 *   <li>Reads those trusted headers via {@link HeaderAuthFilter} into
 *       {@code SecurityContextHolder}.</li>
 *   <li>Permits every request at the Spring-Security filter chain level —
 *       role enforcement happens in (3).</li>
 *   <li>Runs {@link RoleInterceptor} before every {@code /api/**} controller
 *       method to enforce {@link RoleRequired}.</li>
 * </ol>
 *
 * Trust model: this service must be reachable ONLY via the gateway. The
 * gateway strips spoofed X-Auth-* headers before re-injecting its own.
 */
@Slf4j
@Configuration
@EnableWebSecurity
public class SecurityConfig implements WebMvcConfigurer {
    ...
    public static class HeaderAuthFilter extends OncePerRequestFilter {
        ...
        protected void doFilterInternal(HttpServletRequest request,
            ...
            if (user != null && !user.isBlank() && role != null && !role.isBlank()) {
                // Raw role string (no ROLE_ prefix) so Spring's hasAuthority()
                // matches directly. RoleInterceptor below also handles either form.
    ...
    // ────────────────────────────────────────────────────────────────────────
    //  RoleInterceptor — enforces @RoleRequired on controller methods
    // ────────────────────────────────────────────────────────────────────────
    @Slf4j
